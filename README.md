...
- 🔒 **纯本地**：只读取会话文件，绝不写入；服务器仅绑定 `127.0.0.1`（loopback），不监听 `0.0.0.0`/局域网地址，也不下发 `Access-Control-Allow-Origin: *`，跨源网页无法读取或连接本地数据
...
- **仅监听 loopback**：服务器启动时显式绑定 `127.0.0.1`（而非 `0.0.0.0` 等通配地址），局域网内其他设备无法直接访问；端口自增（`--port` 被占用时）同样只在 loopback 上尝试。
- **不下发通配 CORS**：`/api/events`（SSE）等接口不设置 `Access-Control-Allow-Origin: *`，浏览器里的跨源网页即使能发起请求也读不到响应内容。
- **Origin 校验兜底**：请求若带 `Origin` 头，服务器只信任 `http://127.0.0.1:<port>` / `http://localhost:<port>` / `http://[::1]:<port>` 这几种本机地址，其余一律拒绝（403）；无 `Origin` 头的本地非浏览器客户端（如 curl）正常放行。
- 不联网、不上报、不收集任何信息；会话内容仅在内存中解析后通过本地页面展示。
