...
- 🍽 **出餐进餐厅**：厨房外有黄昏小院与餐厅区——菜从出餐口飞上客人餐桌，客人低头扒饭、弹好评、吃完走入夜色
...
- ✅ **自带自测**：`npm test` 43 项断言（回放 / 懒加载 / SSE / demo / 健壮性），前端另有切换系统脚本化自测
...
| `task_complete` | 出餐 ✅（计数 +1，弹 toast + 音效） | 出餐口 → 餐厅区客人餐桌 |
...
- **出餐进餐厅**：北墙窗外是餐厅小院（3 桌 5 座、烛光餐桌）。菜先飞上出餐口窗台，再配送到等菜客人的桌上；客人低头扒饭、弹「好吃！」，吃完起身走入夜色，翻台迎新。歇业时餐厅同步压暗、不再迎客。
- **厨师众生相**：厨师名从 128 个预定义名字里按 threadId 哈希分配（同厨房冲突顺延），不再是任务标题或 id 前缀；制服 8 色、帽型 4 种、围巾、高矮胖瘦、肤色都按 id 确定性派生——同一位厨师每次入职长一个样。
- **黄昏氛围**：厨房外是暮色小院——环形木栅栏、路灯、灌木杂物、远山与亮窗小屋剪影、萤火虫；歇业时整体压暗。
...
- **服务端自测**：`npm test` 跑 `scripts/selftest-server.js`——造 fixture 会话（含坏行 / 半截行 / 全事件类型 / 父子线程 / 懒加载会话组），断言回放快照、回放数量与行数上限、占位厨房按需加载接口（含幂等）、SSE 实时流、demo 模式与畸形输入健壮性，共 43 项断言。
...
- [x] 自测脚本（`npm test` 服务端 43 项断言 + 前端切换系统自测）
- [ ] Claude Code 数据源适配
- [ ] Kimi CLI 数据源适配
...
│   ├── js3d/   kitchen3d / stations / chef / decor / backdrop / dining / fx / audio / palette / textures
│   │           （3D 厨房渲染器 + 暮色背景 + 餐厅区 + WebAudio 合成音效）
│   └── js/     store / net / mock / kitchens-ui / main / selftest / renderer-stub
│               （数据层 + 切换系统 + 前端自测 + 降级渲染器）
├── scripts/selftest-server.js   # 服务端自测（npm test，43 项断言）
