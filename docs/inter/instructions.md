## 2026.07.27

- 11:27 给队友派活应该是跟另外一个厨师交谈
- 11:31 codex 中有项目-会话的概念, 这里也应该体现
![img.png](img.png)
- 11:32 这个板子上的文字效果应该优化 
![img_1.png](img_1.png)
- 11:39 厨房里不应该有那么多在休息的 agent 厨师
- 11:41 主厨的名字不应该是任务的名字
![img_2.png](img_2.png)
- 11:46 想菜单 -> 应该有对应的区域专门去翻阅菜单
![img_3.png](img_3.png)
- 11:47 厨师的名字不应该是一长串字符串, 而是通过 hash 的方式, 映射到指定的厨师名字上(冲突则下一个), 预先定义好 128 个厨师名
- 11:48 不同的厨师应该有不同的形象
- 12:47 去掉跟随最新, 通过项目标签的方式切换不同项目, 注意项目下的任务比较多
![img_4.png](img_4.png)
- 12:48 歇业中的门为什么一直在闪烁, 修复
![img_5.png](img_5.png)
- 12:49 优化厨房外的背景
- 12:59 写一个启动脚本, 关闭之前的无用进程端口, 重新启动项目
- 13:00 协作工具（send_message / wait_agent / followup_task 等）	给队友传话 / 等队友 💬	应该是要跟其他厨师交互的
- 13:02 更新 readme
- 13:11 主要是主厨在做, 为什么每次做完一件事情后, 主厨都会回到原位, 不能继续做么, 或者喝口水, 休息会儿, 连贯点
- 14:56 优化网页启动时, 直接拉取了太多消息的问题, 是否可以只拉取一个会话的最近100条, 默认只拉取最近5个会话, 其他会话点击后再拉取
- 15:05 做出来的菜, 应该有餐厅, 有客人使用
- 15:06 增加炒菜的区域
- 

